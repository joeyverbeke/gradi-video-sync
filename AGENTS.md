...
- `docs/` – reference material, e.g., `raspi_vlc_sync_plan.md` for the phased rollout plan.
- `scripts/` – automation helpers such as `phase1_validate.sh` (hardware validation on a Pi).
...
- `bash scripts/phase1_validate.sh /path/to/video.mp4` – checks GPU overlays, kernel modules, packages, and video metadata before running the manual VLC observation.
...
- Device/video naming follows the `pi-{front,back}-{a,b}` and `{front,back}_{a,b}.mp4` patterns defined in `docs/raspi_vlc_sync_plan.md`.

## Testing Guidelines
- Scripts should include self-check output (e.g., pass/warn/fail summaries) and exit non-zero on critical failures.
- Manual verification (VLC playback, CPU observation) must be recorded in the verification log table at the bottom of the plan.
...
- Pull requests should describe the phase or feature targeted, list manual test evidence (e.g., `phase1_validate.sh` output), and link any tracking issues.
